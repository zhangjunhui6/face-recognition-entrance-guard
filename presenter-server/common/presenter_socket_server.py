...
# epool will return if no event coming in 2 s
EPOLL_TIMEOUT = 2
...
class PresenterSocketServer:
    ...
    def __init__(self, server_address):
        """
        Args:
            server_address: server listen address(ipv4,port)
        """
        ...
        try:
            ...
            while True:
                ...
                for sock_fileno, event in events:
                    ...
                    # it means presenter agent exit without close socket.
    ...
    # accept new socket from client,join to conns,and epoll register it
    ...
    # parse msg head(total len and name len)
    ...
    # read msg body,save in msgs[sock_fd]
    def _read_msg_body(self, sock_fd, conns, msg_body_len, msgs):
        """
        Args:
            sock_fd: a socket fileno
            conns: all socket connections which created by server.
            msg_body_len: message body length.
            msgs: msg read from a socket
        Returns:
            ret: True or False
        """
    ...
    def send_message(self, conn, protobuf, msg_name):
        ...
        # in Struct(), 'I' is unsigned int, 'B' is unsigned char
        s = struct.Struct('IB')
        msg_head = (socket.htonl(msg_total_size), msg_name_size) # 包头是元组类型，其中消息总长度被字节序列
        packed_msg_head = s.pack(*msg_head) # 打包结构体
        msg_data = packed_msg_head + \
                   bytes(msg_name, encoding="utf-8") + message_data # 消息名被encode
