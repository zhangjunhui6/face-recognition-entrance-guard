...
namespace ascend {
    namespace presenter {
          ...
          SharedByteBuffer MessageCodec::EncodeMessage(const PartialMessageWithTlvs& msg) {
            ...
            encode_size += msg_name_size + msg_size; // + 名称长度 + 数据长度

            uint32_t total_size = encode_size;
            // if has additional field 加上每一个tlv所占的长度(3部分)
            if (!tlv_list.empty()) {
          ...
          buffer.PutUInt32(total_size); //总长度
          buffer.PutUInt8(msg_name_size); // 消息名称长度
          buffer.PutString(name); // 名称
          // serialization may fail if any of the required field is not set,
          // in which case, a empty buffer is returned
          if (!buffer.PutMessage(message)) {  // message序列化
            return SharedByteBuffer();
          }
